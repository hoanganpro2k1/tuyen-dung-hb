...
export const DetectionSlider: React.FC<DetectionSliderProps> = ({ slides }) => {
  ...
  return (
    <div className="container relative z-10 mx-auto px-4">
      <div className="mb-8 flex flex-col md:flex-row items-center justify-center gap-3 text-white">
        <SearchX className="h-6 w-6 md:h-8 md:w-8 text-gold-primary" />
        <h2 className="text-xl font-bold tracking-wide md:text-[36px] text-center">Khám phá khoá học của Học Bá</h2>
      </div>
      ...
      <div className="mb-12 flex items-center justify-center gap-4">
        {slides.map((slide, index) => {
          ...
          return (
            ...
            >
              <h3 className="relative z-10 uppercase">{slide.label}</h3>
              {isActive && (
                <div
                  className={cn(
      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <div className="mx-auto shadow-2xl overflow-hidden rounded-[2rem] bg-white border border-gold-primary/10" ref={emblaRef}>
          <div className="flex">
            {slides.map((slide, index) => (
              <div
                key={slide.id}
              ...
              >
                <div
                  className={cn(
                    'group flex flex-col md:flex-row items-center p-6 md:p-10 lg:p-16 transition-all duration-500',
                    selectedIndex === index ? 'opacity-100' : 'opacity-0 md:opacity-0',
                  )}
                >
                  <div className="relative mb-8 md:mb-0 md:mr-12 lg:mr-20 flex-shrink-0">
                    <div className="relative h-64 w-64 md:h-80 md:w-80 lg:h-[336px] lg:w-[346px] transition-transform duration-700 group-hover:scale-105">
                      <Image
                        src={slide.image}
                        alt={slide.label}
                        fill
                  ...
                  <div className="flex-grow flex flex-col items-center md:items-start text-center md:text-left">
                    <p
                      className="mb-8 text-base md:text-[20px] font-medium leading-relaxed text-[#373737]"
                      dangerouslySetInnerHTML={{ __html: slide.description }}
                    />
                    <Link href={slide.href} className="px-10 py-4 font-bold shadow-gold/30">
